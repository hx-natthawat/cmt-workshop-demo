...
#   3. MCP showcase : tools/list ครบ 7 + resource store://policy + prompt after_sales_reply
...
if [ ! -d s2-mcp/showcase/node_modules ]; then
  ...
  MISSING=""
  for t in recommend_for_skin track_order check_stock create_draft_order get_bestsellers get_promotions confirm_order; do
    echo "$OUT" | grep -q "\"$t\"" || MISSING="$MISSING $t"
  done
  [ -z "$MISSING" ] && ok "tools/list ครบ 7 tools" || bad "tools/list ขาด:$MISSING" "เช็ค showcase/server.mjs"
